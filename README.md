...
makes a copy of it on your computer since it isn't possible to run it on GitHub
To run the game, either run `uv run main.py` (again, in your terminal) or open Finder and double-click run.sh. Both of the ways will open a new window with the game. 

## More Installation
Run this command to install homebrew: `/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"`. After that, run `brew install neovim` . If you're in your home directory (the command line has ~ near the end, which you probably are, if you're not, run `cd ~`), run `ls -a`. Then open your .zshrc or .bashrc file (`nvim <FILE>`) (Note: replace `<FILE>` with .bashrc or .zshrc) and press the i key to enter insert mode. Then add this line to the bottom: `export PATH="/opt/homebrew/bin:$PATH"`. Then press escape and type this `:wq`. That will exit out of the file and save it. Finally, run `brew install uv python git` and `uv pip install pygame`
...
Also, when the player lands on a planet the screen goes blank. Don't worry, all you have to do to is press L to launch again
